...
[1] International Monetary Fund, Global price of Dubai Crude [POILDUBUSDM], 
retrieved from FRED, Federal Reserve Bank of St. Louis; 
https://fred.stlouisfed.org/series/POILDUBUSDM, June 8, 2023.
...
# Read Dubai Crude monthly average price data [2]
data_path = "data/POILDUBUSDM.csv"
...
"""
data_dir = "data"
output_filename = "dubai_prices_monthly_raw.csv"
...
data['POILDUBUSDM'] = pd.to_numeric(data['POILDUBUSDM'], errors='coerce')
...
ax.set_title("Dubai Crude Monthly Average Price")
ax.set_xlabel("Date")
ax.set_ylabel("USD")
...
fig.savefig("results/figures/dubai_monthly_avg_price.png")

# TODO (Pranay): This is redundant for monthly raw data. Remove it. For now, keep for reproducibility to paper. It won't affect results much.
""" 
This resamples monthly data to be monthly. It takes the first value to maximize
the number of data points. Then it computes the excess returns percentage,
which is used as a proxy for volatility.
"""
resampled_data = data.resample('M').last()
percent_returns_data = resampled_data.pct_change().dropna() * 100 # Get monthly returns in percent change
...
fig.savefig("results/figures/dubai_month_excess_returns_percent.png")
...
fitted to the monthly excess return data. Note that for any given time step, the sum
...
fig.savefig("results/figures/dubai_month_task_smooth_probs.png")
...
fig.savefig("results/figures/dubai_month_task_labels.png")
...
axes[0].plot(x, y)
axes[0].set_title("Dubai Crude Oil Monthly Average Price")
...
fig.savefig("results/figures/dubai_all_plots.png")

"""
This outputs resampled data and corresponding task labels.
"""
data_copy['DATE_DT'] = pd.to_datetime(data_copy['DATE'])
data_copy['POILDUBUSDM'] = pd.to_numeric(data_copy['POILDUBUSDM'], errors='coerce')
data_copy = data_copy.set_index('DATE_DT')
data_copy = data_copy.drop('DATE', axis=1)

data_copy_resampled_monthly = data_copy.resample('M').last().dropna()

data_copy_resampled_monthly = data_copy_resampled_monthly.iloc[1:].squeeze().reset_index()

data_copy_resampled_monthly['TASK'] = labels

data_dir = "results/transformed_data"
output_filename = "dubai_prices_tasks_resamp_month.csv"
output_filepath = os.path.join(data_dir, output_filename)
if not os.path.exists(data_dir):
    os.mkdir(data_dir)
